...
class TarShardDataset(Dataset):
    ...
    def __init__(self, data_dir: Path, split: str, augment: bool = False,
                 no_colorjitter: bool = False) -> None:
        self.split   = split
        ...
        self._photometric = None if no_colorjitter else T.RandomApply(
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--no-colorjitter", action="store_true",
                   help="Disable ColorJitter augmentation (keep only RandomAffine)")
...
def main() -> None:
    ...
    ds_train = TarShardDataset(args.data_dir, split="train", augment=True,
                               no_colorjitter=args.no_colorjitter)
